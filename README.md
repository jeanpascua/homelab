...
Personal homelab on a Lenovo M710q mini-PC ($110 CAD total). Running Proxmox VE as the hypervisor with Ubuntu Server, Kali Linux, and Metasploitable VMs.
...
Linux admin, Proxmox/KVM, Docker, DNS (Pi-hole, split DNS, systemd-resolved), VPN (Tailscale/WireGuard), SSH, reverse proxy (Nginx Proxy Manager), Grafana/Prometheus monitoring, Syncthing, Claude Code + MCP, SSL/TLS
